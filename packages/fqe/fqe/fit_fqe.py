...
class FQEModel:
    ...
    def predict_q(self, states: Array, actions: Array) -> Array:
        ...
        return _predict_booster(self.booster, _state_action_features(states, actions, self.state_dim, self.action_dim))
    ...
    *,
    ...
    for iteration in iterator:
        if iteration % config.update_target_frequency == 0:
            train_targets = _clip_targets(
                rewards[train_idx] + gamma * (1.0 - terminals_1d[train_idx]) * next_pred_train,
                target_min,
                target_max,
            )
        ...
        next_pred_train_candidate = _predict_next_average(booster, next_features[train_idx])
        next_pred_val_candidate = _predict_next_average(booster, next_features[val_idx])
        ...
        if improved:
            ...
            if config.refit_on_all_data:
                all_targets = _clip_targets(
                    rewards + gamma * (1.0 - terminals_1d) * next_pred_all,
                    target_min,
                    target_max,
                )
                ...
                next_pred_all = _predict_next_average(booster_all, next_features)
        else:
            patience += 1
...
def _fit_constant_fixed_point(
    *,
    ...
    for iteration in range(config.num_iterations):
        target = _clip_targets(
            rewards + float(gamma) * (1.0 - terminals) * value,
            config.target_min,
            config.target_max,
        )
...
def _state_action_features(states: Array, actions: Array, state_dim: int, action_dim: int | None) -> Array:
    ...
    actions_2d = _as_2d_float(actions, "actions", n_rows=states_2d.shape[0], expected_cols=action_dim)
...
def _predict_next_average(booster: Any, next_features: Array) -> Array:
    n, m, d = next_features.shape
    flat_pred = _predict_booster(booster, next_features.reshape(n * m, d))
    return flat_pred.reshape(n, m).mean(axis=1)
...
def _bellman_risk(
    *,
    ...
    return float(np.average(err * err, weights=sample_weight))
