...
CRITIC_LABELS = {"neural_fqe": "Neural FQE", "rf_fqe": "RF FQE"}
METRICS = {
...
def _is_valid_run(run: RunInfo, *, strict_expected: bool = True) -> bool:
    ...
    for family in ["neural_fqe", "rf_fqe"]:
...
def _plot_main(summary: pd.DataFrame, unit: pd.DataFrame, manifest: dict[str, object], figures_dir: Path) -> None:
    ...
    fig, (ax0, ax1) = plt.subplots(1, 2, figsize=(5.8, 2.65), gridspec_kw={"width_ratios": [1.08, 1.0]})
...
def _plot_audit(summary: pd.DataFrame, figures_dir: Path) -> None:
    ...
    fig, axes = plt.subplots(1, 4, figsize=(7.6, 3.6), sharey=True)
    ...
    fig.suptitle("Full Hopper Q-calibration audit: ratios below one improve on raw critics", fontsize=9, fontweight="bold")
...
def _write_numbers(
    ...
    n_units = int(manifest.get("n_completed_unit_files", 0))
