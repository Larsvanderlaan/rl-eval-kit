...
ratios---one for the target initial distribution and one for the one-step target
distribution---and then solves the adjoint Bellman equation by iterating
supervised one-step adjoint regressions. This formulation replaces coupled
minimax optimization with standard prediction subproblems, making occupancy-ratio
estimation compatible with off-the-shelf supervised learners and post-update
stabilization. We show that the population adjoint Bellman update is
contractive in total variation and has the discounted occupancy ratio as its
unique fixed point. We then prove modular finite-sample bounds that separate
preliminary density-ratio error, one-step adjoint-regression error,
approximation and excess-risk terms, stabilization error, and finite-iteration
bias, yielding transparent error propagation for fitted occupancy-ratio
estimation.
\end{abstract}
...
\citep{thomasBrunskill2016DataEfficientOPE,jiangLi2016DoublyRobust,xieEtAl2019MarginalizedIS,yinWang2020EfficientTabularOPE,kallusUehara2020DoubleRL,liuEtAl2018InfiniteHorizonOPE,hallakMannor2017COPTD,suttonEtAl2016EmphaticTD,geladaBellemare2019CovariateShift,vanDerLaanKallus2025StationaryWeightedFQE,vanDerLaanKallus2025SoftFQI}.
...
Bellman moment restrictions through coupled optimization over ratios and critics
\citep{nachumEtAl2019DualDICE,zhangEtAl2020GenDICE,
zhangEtAl2020GradientDICE,yangEtAl2020RegularizedLagrangianDICE,
daiEtAl2020CoinDICE,ueharaEtAl2020MWLMQL}. These methods are principled and
influential, but their saddle-point objectives combine the ratio model, critic
class, regularization, numerical stabilization, and optimization procedure in a
single training problem. In practice, this structure is most naturally
implemented with linear models or differentiable neural networks, where joint or
alternating gradient updates are readily available. It is less direct to use
off-the-shelf supervised learners such as gradient-boosted trees and random
forests, whose strengths come from standard predictive losses and mature
training routines.
...
equation. \textsc{FORI} exploits this fixed-point structure by first estimating
two preliminary density ratios---one for the target initial distribution and
one for the one-step target distribution---and then iterating fitted one-step
adjoint regressions followed by adjoint Bellman updates. Stabilization
...
The analogy with fitted Q evaluation (FQE) is direct. FQE estimates a target
value function by iterating fitted Bellman regressions; \textsc{FORI} estimates
the discounted occupancy ratio by iterating fitted adjoint Bellman regressions.
Thus occupancy-ratio estimation can be organized as a sequence of standard
prediction problems while preserving the dynamic equation that defines the
target ratio.
...
preliminary density-ratio error, one-step adjoint-regression error, projection
and excess-risk terms, stabilization error, and finite-iteration bias. The
resulting bound separates the effects of estimating the initial and one-step
density ratios, temporal propagation, function approximation, finite-sample
regression, and post-update stabilization.
...
Classical off-policy evaluation reweights returns using trajectory or
per-decision importance ratios, while doubly robust estimators combine
importance weighting with value-function estimates to reduce variance and retain
bias correction under nuisance-estimation error
\citep{thomasBrunskill2016DataEfficientOPE,jiangLi2016DoublyRobust}.
Marginalized importance sampling replaces trajectory products by marginal state
or state-action occupancy ratios, yielding finite- and infinite-horizon
estimators based on distributional corrections
\citep{xieEtAl2019MarginalizedIS,yinWang2020EfficientTabularOPE,
liuEtAl2018InfiniteHorizonOPE}. Double reinforcement learning makes this
connection explicit: efficient and doubly robust OPE estimators combine a
value-function nuisance with a marginal density-ratio nuisance, making
occupancy-ratio estimation a central ingredient in semiparametric OPE
\citep{kallusUehara2019IntrinsicOPE,kallusUehara2020DRL_ICML,
kallusUehara2020DoubleRL,ueharaShiKallus2022OPEReview}. Related
semiparametric work develops efficient and doubly robust estimators for policy
gradients, deterministic policies, horizon-robust OPE, natural policy targets,
and policy-learning or inverse-reinforcement-learning functionals
\citep{kallusUehara2020PolicyGradients,kallusUehara2020DeterministicPolicyDR,
kallusUehara2022BreakingHorizonDRL,kallusUehara2024NaturalPolicies,
vanDerLaanEtAl2025AutomaticDRL,vanDerLaanKallusBibaut2025ClassificationIRL,
vanDerLaanBibautKallus2025EfficientIRL}. Stationary-distribution corrections also appear in consistent off-policy TD,
emphatic weighting, and covariate-shift bootstrapping for value learning
\citep{hallakMannor2017COPTD,suttonEtAl2016EmphaticTD,
geladaBellemare2019CovariateShift,pattersonEtAl2022GeneralizedPBE}. More
recently, stationary weighting has been used to stabilize fitted value methods
such as FQE and FQI by aligning Bellman regression with the target stationary
distribution, especially when Bellman completeness is absent
\citep{vanDerLaanKallus2025StationaryWeightedFQE,vanDerLaanKallus2025SoftFQI}. \textsc{FORI}
contributes to this line of work by estimating state-action occupancy
corrections through fitted adjoint-Bellman iteration, using one-step
density-ratio estimation and supervised one-step adjoint regression.
...
or value functions
\citep{lagoudakisParr2003LSPI,ernstEtAl2005TreeBatchRL,antosEtAl2007FQI,
munosSzepesvari2008FVI,leEtAl2019BatchPolicyConstraints}.
...
\citep{vanDerLaanKallus2025StationaryWeightedFQE,
vanDerLaanKallus2025SoftFQI,pattersonEtAl2022GeneralizedPBE}.
\textsc{FORI} uses the same fitted-iteration template, but applies it to the
adjoint Bellman equation for occupancy ratios. The fitted recursion therefore
estimates the density correction that reweights the offline distribution toward
the target-policy discounted occupancy distribution, rather than a value
function.

\paragraph{Density-ratio estimation.}
\textsc{FORI} treats the preliminary density-ratio estimates as modular
first-stage inputs. In particular, the one-step density ratio
\(\cpi=\dd(\nu\Ppi)/\dd\nu\) can be estimated by distinguishing one-step
target-policy successor samples from reference samples. This connects
\textsc{FORI} to the broad literature on direct density-ratio estimation for
covariate shift, including KLIEP and related likelihood-ratio objectives,
kernel mean matching, least-squares importance fitting, unconstrained LSIF,
convex-risk and \(f\)-divergence formulations, and Bregman density-ratio losses
\citep{huangEtAl2006KMM,tsuboiEtAl2008KLIEP,kanamoriEtAl2009LSIF,
nguyenEtAl2010ConvexRatio,gutmannHirayama2011Bregman,
sugiyamaEtAl2012BregmanDensityRatio,sugiyamaEtAl2012DensityRatioBook,
hines2025learning}. Density ratios can also be estimated through
probabilistic classification, by discriminating numerator samples from
reference samples and converting the fitted class probability into odds
\citep{qin1998Inferences,chengChu2004SemiparametricDensity,
bickelEtAl2008Multitask,menonOng2016DensityRatioClassification}.
Our theory enters these first-stage choices only through explicit initial and
one-step density-ratio error terms, so the fitted adjoint Bellman iteration is
unchanged by the particular density-ratio learner used.
...
The following representation is the density identity behind the fitted update.
\begin{theorem}[One-step regression representation]
\label{thm:reverse-regression}
Assume the overlap condition in Assumption~\ref{ass:overlap}. For every
$\omega\in L^1(\nu)$, $\Mpi\omega$ is integrable under $\nup$ and
\[
    \frac{\dd((\omega\nu)\Ppi)}{\dd\nu}
    =
    \cpi\,\Mpi\omega,
...
Thus a single target-policy propagation of the weighted data law $\omega\nu$
can be written as a one-step adjoint regression, $\Mpi\omega$, followed by
multiplication by the one-step density ratio $\cpi$. This motivates the adjoint
Bellman update
\begin{equation}
\label{eq:operator}
    \Bpig\omega
    ...
    (1-\gamma)\omegazero+\gamma\cpi\Mpi\omega .
\end{equation}
Using the discounted occupancy recursion
$d_{\pi,\gamma}=(1-\gamma)\dinit+\gamma d_{\pi,\gamma}\Ppi$, the discounted
occupancy ratio satisfies the fixed-point equation
\begin{equation}
...
The fitted iteration takes as input estimates of the two density ratios defined
above,
\[
    \omegazero=\frac{\dd\dinit}{\dd\nu},
    \qquad
    \cpi=\frac{\dd(\nu\Ppi)}{\dd\nu}.
\]
Both are ordinary density-ratio estimation problems with reference law \(\nu\).
Thus \(\textsc{FORI}\) separates the dynamic propagation step from the
first-stage problem of estimating two local density ratios.

For concreteness, let \(\rho\) be any numerator law and define
\(r_\rho=\dd\rho/\dd\nu\). A least-squares density-ratio estimator minimizes,
over a chosen function class,
\[
    ...
    \int r(x)\,\rho(\dd x).
\]
Since \(\dd\rho=r_\rho\,\dd\nu\), its excess risk is
\[
    \mathcal L_{\mathrm{LS}}(r)-\mathcal L_{\mathrm{LS}}(r_\rho)
    =
    \frac12 \|r-r_\rho\|_{2,\nu}^2 .
\]
Thus the unrestricted population minimizer is \(r_\rho\), and the loss directly
targets \(L^2(\nu)\) density-ratio error. Since \(\nu\) is a probability
measure, this also controls \(L^1(\nu)\) error through
\(\|r-r_\rho\|_{1,\nu}\le \|r-r_\rho\|_{2,\nu}\). This norm compatibility is
useful below, where first-stage density-ratio errors enter the fitted-iteration
analysis.

In practice, the integrals in \(\mathcal L_{\mathrm{LS}}\) are replaced by
empirical averages. For \(\cpi\), the numerator sample is the one-step target
sample \(\{X_i^+\}_{i=1}^n\), while the reference sample is the observed sample
\(\{X_i\}_{i=1}^n\). For \(\omegazero\), the numerator sample is drawn from the
target initial state-action law \(\dinit\), when such samples are available.

The least-squares choice is not essential. Other density-ratio learners can be
used without changing the fitted adjoint Bellman iteration, provided their
statistical guarantees are translated into the \(L^1(\nu)\) or \(L^2(\nu)\)
first-stage errors used below. Thus \(\textsc{FORI}\) is modular in the
learners used to construct \(\omegahatzero\) and \(\chat\).

\subsection{Algorithm}

Given first-stage estimates $\omegahatzero\approx\omegazero$ and
$\chat\approx\cpi$, \textsc{FORI} applies the adjoint Bellman equation by fitted
iteration. Starting from an initial estimate
...
The one-step adjoint regression need not be solved to completion at every
iteration. Neural implementations can warm-start \(\mhat_k\) from the previous
iterate and take a small number of stochastic-gradient steps on the supervised
regression loss. Tree-based implementations can add a few boosting stages or
refit an off-the-shelf supervised learner to the current pseudo-outcomes. Thus
\(\textsc{FORI}\) has the same approximate fitted-iteration character as neural
fitted \(Q\)-iteration and boosting-style fitted value iteration
\citep{riedmiller2005neural,tosatto2017boosted}. For clarity, our explicit
finite-sample instantiation analyzes fully fitted sample-split least-squares
regressions; the general perturbation theorem only requires control of the
realized one-step adjoint-regression error, so approximate inner solves are
covered whenever that error is controlled.
...
\subsection{Population Contraction and Fixed Point}

The representation in Theorem~\ref{thm:reverse-regression} is the population
identity behind Algorithm~\ref{alg:fori}. It also implies that the population
update propagates signed measures through the target-policy kernel, yielding the
...
We developed \textsc{FORI}, a fitted-iteration approach to discounted
occupancy-ratio estimation. The central idea is to view the discounted
occupancy ratio as the fixed point of an adjoint Bellman equation, and to solve
this equation using supervised one-step adjoint regressions rather than a
coupled ratio--critic saddle-point objective. This gives occupancy-ratio
estimation the same modular structure that fitted Bellman methods provide for
value-function estimation.

The resulting algorithm separates the problem into interpretable components:
first-stage estimation of the initial and one-step density ratios, fitted
one-step adjoint regression, temporal propagation through the adjoint Bellman
update, and optional post-update stabilization. Our analysis shows that the
population update is contractive in total variation and that fitted
implementations inherit a perturbation bound separating first-stage
density-ratio error, adjoint-regression error, approximation and excess-risk
terms, stabilization error, and finite-iteration bias.

This perspective does not remove the need for overlap or accurate nuisance
estimation. Rather, it makes these requirements explicit and modular.
\textsc{FORI} allows different density-ratio estimators and supervised learners
to be plugged into the same adjoint fixed-point iteration, making it a flexible
alternative to minimax occupancy-ratio estimation. More broadly, the paper
suggests that occupancy correction can be organized around the same fitted
supervised-learning principles that have made fitted Bellman methods practical
in offline reinforcement learning.


\paragraph{Fitted iteration versus minimax weighting.}
The choice between \textsc{FORI} and DICE-style or minimax weight estimation is
analogous to the choice between fitted Bellman regression and minimax
Bellman-residual methods for value estimation. Minimax formulations directly
enforce global Bellman moment restrictions and can be statistically attractive
when the ratio and critic classes are well matched and the saddle-point problem
is reliably optimized. Their practical performance, however, depends on coupled
choices of ratio model, critic class, regularization, stabilization, and
min--max optimization. \textsc{FORI} makes a different tradeoff: it replaces the
global saddle-point problem by local density-ratio estimation and supervised
one-step adjoint regression. This modularity makes it easier to use mature
off-the-shelf learners and to diagnose separate sources of error, but it still
requires adequate overlap and accurate first-stage and regression fits. Thus
\textsc{FORI} is best viewed not as a uniformly dominant replacement for
minimax weighting, but as the fitted-regression counterpart: a simpler and more
modular route to the same discounted occupancy-ratio fixed point.
...
Practical diagnostics should therefore include value error, effective sample size, mass after normalization, clipping frequency, and upper-tail summaries of $\chat$ and $\omegahat^{(k)}$.
