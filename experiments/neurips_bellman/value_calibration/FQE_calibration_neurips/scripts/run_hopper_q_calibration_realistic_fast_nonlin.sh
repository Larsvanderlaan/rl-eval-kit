...
OUT_DIR="FQE_calibration_neurips/results/hopper_q_calibration_realistic_fast_nonlin_hopper512_s5"
LOG_DIR="FQE_calibration_neurips/results/logs/realistic_fast_nonlin_hopper512_s5"
mkdir -p "${LOG_DIR}"
LAST_PID=""
...
common_args=(
  ...
  --min_bin_size 100
  --metric_bins 20
  --metric_min_bin_size 100
  --action_samples 4
  --initial_action_samples 8
  --output_dir "${OUT_DIR}"
)
...
run_shard_bg() {
  ...
  local start="$2"
  local stop="$3"
  echo "[realistic-fast] start ${label} $(date)" >&2
...
echo "[realistic-fast] launched $(date)"
...
wait "${pid2}" || failed=1
if [ "${failed}" -ne 0 ]; then
  echo "[realistic-fast] shard failure; see ${LOG_DIR}" >&2
  exit 1
fi
...
echo "[realistic-fast] completed $(date)"
